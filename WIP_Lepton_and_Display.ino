...
uint16_t getThermalColor(double normVal) {
  normVal = constrain(normVal, 0.0, 1.0);
  int r = normVal * 255;
  int g = 0;
  int b = (1.0 - normVal) * 255;
...
void extractImageData(int *flatImage) {
  int index = 0;
  for (int y = 0; y < 120; y++) {
    for (int x = 0; x < 160; x++) {
      flatImage[index++] = image[x][y];
    }
  }
}
...
void loop() {
  ...
  // Calcul des valeurs min et max directement depuis image[x][y]
  ...
  // Affichage pixel par pixel avec normalisation
  for (int y = 0; y < IMAGE_HEIGHT; y++) {
    for (int x = 0; x < IMAGE_WIDTH; x++) {
      double norm = (image[x][y] - minVal) / (maxVal - minVal);
      uint16_t color = getThermalColor(norm);

      // Optionnel : masque une zone
  ...
  // Affichage min/max en bas de lՎcran
  tft.setCursor(0, 122);
  tft.setTextColor(ST77XX_WHITE, ST77XX_BLACK);
  tft.setTextSize(1);
  tft.print("Min: ");

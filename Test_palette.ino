...
void displayImage(void) {
  ...
  for (int i = 0; i < image_x; i++) {
    for (int j = 0; j < image_y; j++) {
      ...
      int index = constrain(val * 255, 0, 255);  // Index dans la palette

      uint8_t r = colormap_rainbow[index * 3];
      uint8_t g = colormap_rainbow[index * 3 + 1];
      uint8_t b = colormap_rainbow[index * 3 + 2];

      int color = ips.color565(r, g, b);

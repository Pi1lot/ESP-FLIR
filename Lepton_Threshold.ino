...
void displayImage(void) {
  ...
  for (int i = 0; i < image_x; i++) {
    for (int j = 0; j < image_y; j++) {
      double val = (image[i][j] - min) / (max - min); // val entre 0.0 et 1.0

      uint8_t r, g, b;

      if (val < 0.85) {
        // Niveaux de gris jusquՈ 85% de lՎchelle
        uint8_t gray = val * (255.0 / 0.85);
        r = g = b = gray;
      } else {
        // Rouge vif pour les 15% les plus chauds
        r = 255;
        g = b = 0;
      }
